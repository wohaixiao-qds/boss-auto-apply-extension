...
import { parseAiResponseBody } from "./agent/ai-response";
    ...
    case "RESOLVE_APPROVAL": {
      ...
      return respond(chrome.storage.local.get({ pendingApproval: null, agentRecovery: null }).then(({ pendingApproval, agentRecovery }) => {
        ...
        const nextMirror: AgentRecoveryMirror = {
          ...
          currentGreetIndex: 0,
          greetListUrl: baseMirror.greetListUrl || ""
        };
    ...
    case "RESOLVE_UNKNOWN_GREET": {
      ...
      return respond(chrome.storage.local.get({ agentRecovery: null as AgentRecoveryMirror | null }).then(({ agentRecovery }) => {
        ...
        const nextMirror: AgentRecoveryMirror = {
          ...
          greetListUrl: base.greetListUrl || ""
        };
...
async function callAi(settings: Settings, payload: Record<string, unknown>): Promise<any> {
  ...
  try {
    ...
    const data = parseAiResponseBody(body, response.status, response.headers.get("content-type") || "") as any;
...
Phase A（screen，筛选阶段）：先检查 state.appliedFilters 是否包含 url_filters_applied；若包含，URL 参数已经完成首轮筛选，不要重复点击筛选 chip，先观察当前 URL/页面 chip 是否与 effectiveQuery 一致，再 collect_jobs（必要时先 next_page）→ filter_jobs → rank_jobs → request_greet_approval。若 URL 没覆盖某个条件，再用 click/fill 处理剩余条件；如果 page.kind=jobs 且 URL 已应用，即使 screen 中没有 [eN]，也必须先尝试 collect_jobs，不要因为没有 ref 直接 pause；不要自行打招呼。

匹配分只用于岗位排序和展示，不是硬筛选条件；不得因为分数低而 pause，也不得要求用户设置最低匹配分。
...
Phase B（greet，打招呼阶段）：BOSS 列表、右侧详情和“立即沟通”在同一个页面。Runtime 必须点击左侧岗位卡片并确认右侧出现“立即沟通”后，才把控制权交给 LLM；不得点击详情链接或打开新的详情页面。Phase B 的 page.screen 只会暴露沟通控件和聊天输入框，LLM 只能从其中选择 ref，严禁使用筛选或普通岗位 ref。LLM 在岗位切换完成后只负责点击“立即沟通”，不得再次点击普通岗位卡片。点击动作成功即视为当前岗位打招呼完成；若出现“已向BOSS发送消息”确认弹窗，关闭后继续。当前岗位完成后由 Runtime 留在列表页点击下一岗位，不要自行导航到下一个岗位。
